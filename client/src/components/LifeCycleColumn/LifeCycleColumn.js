...
import { reducer, ADD_NEW_ISSUE, REORDER_ISSUE } from '../../reducer/reducer';
...
const LifeCycleColumn = ({ title, issues = [] }) => {
  ...
  const handleOnSubmit = newIssues => {
    console.log("issues title:",newIssues);
    dispatch({ type: ADD_NEW_ISSUE, payload: { title: newIssues, lifeCycleName: title }, boardID : '100' });
  };
